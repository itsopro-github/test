<?php
	class UploadComponent extends Object {
		...
		function upload ($file, $destination, $name = NULL, $rules = NULL, $allowed = NULL) {
			...
			if (!is_null($allowed)) { $this->_allowed = $allowed; } else { $this->_allowed = array('jpg','jpeg','gif','png','bmp','pdf','odt','doc','docx','rtf','ods','xls','xlw','odp','ppt','pps','zip','txt','csv','html','xml'); }
		 ...
		 if (is_null($type)) $type = 'resize';
			...
			switch ($type) {
				...
				case 'resizedimension':
					...
					if ($uploadWidth > $maxW) {
						$newX = $maxW;
					}	else if ($uploadWidth < $maxW) {
						$newX = $uploadWidth;
					}

					if ($uploadHeight > $maxH) {
						$newY = $maxH;
					}	else if ($uploadHeight < $maxH) {
						$newY = $uploadHeight;
					}
